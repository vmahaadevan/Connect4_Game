$(document).ready(function() {
	...
	var soundHandle = document.getElementById('piece_drop');
		soundHandle.src = 'sounds/piece-drop.wav';
	...
	$('.droppable').droppable({
        ...
        drop: function(event, ui) {
			...
			if ($(ui.draggable)) {
				...
				var current_color = $(ui.draggable).hasClass('red_chip') ? 'red_chip' : 'black_chip';
				var next_color = $(ui.draggable).hasClass('red_chip') ? 'black_chip' : 'red_chip';
				...
				if ($('#buttons_holder').children().length < 2) {
					$('#buttons_holder').append('<input id="undo" type="button" value="Undo Last Move">');
				}
